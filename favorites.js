// ========================================
// FAVORITES/WISHLIST SYSTEM - FULLY FIXED VERSION
// ========================================
    ...
    if (favorites.length === 0) {
        content.innerHTML = `
            <div class="empty-favorites">
                ...
                <p>เพิ่มสินค้าที่คุณชอบเข้ามาได้เลย</p>
                <button class="btn-modal-primary" onclick="closeModal(); document.getElementById('products').scrollIntoView({behavior: 'smooth'})">
                    🛍️ เลือกสินค้า
                </button>
            </div>`;
    } else {
        ...
        favoriteProducts.forEach(product => {
            html += `
                <div class="favorite-item">
                    <button class="btn-remove-favorite" onclick="toggleFavorite(${product.id}, '${product.category}', event)">✕</button>
                    <div class="favorite-emoji">${product.emoji}</div>
                    <div class="favorite-name">${product.name}</div>
                    <div class="favorite-price">${product.price.toLocaleString()} ฿</div>
                    <button class="btn-add-to-cart-favorite" onclick="addToCart(${product.id}, '${product.category}'); showNotification('เพิ่มลงตะกร้าแล้ว', 'success')">
                        🛒 เพิ่มลงตะกร้า
                    </button>
                </div>`;
        });
        ...
        html += `
            <div class="favorites-actions">
                <button class="btn-modal-primary" onclick="addAllFavoritesToCart()">🛒 เพิ่มทั้งหมดลงตะกร้า</button>
                <button class="btn-settings-secondary" onclick="clearAllFavorites()">🗑️ ล้างรายการโปรด</button>
            </div>`;
